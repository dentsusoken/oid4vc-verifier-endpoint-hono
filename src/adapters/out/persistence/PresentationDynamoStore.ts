 ...
 */
export class PresentationDynamoStore {
  /**
   * Creates an instance of PresentationKVStore.
   * @param {DynamoDB} dynamo - The DynamoDB client to use for storage.
   */
  ...
  loadPresentationById: LoadPresentationById = async (id: TransactionId) => {
    const jsonStr = await this.dynamo.get(idKey(id.value));
  ...
  loadPresentationByRequestId: LoadPresentationByRequestId = async (
    ...
    const id = await this.dynamo.get(requestIdKey(requestId.value));
  ...
  storePresentation: StorePresentation = async (presentation: Presentation) => {
    ...
    await this.dynamo.put(idKey(presentation.id.value), jsonStr, putOptions);
