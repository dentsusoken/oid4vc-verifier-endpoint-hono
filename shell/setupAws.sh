...
# Lambda関数の作成または更新
echo "Checking if Lambda function exists: verifier-endpoint"
LAMBDA_EXISTS=$(aws lambda get-function --function-name verifier-endpoint --region $AWS_DEFAULT_REGION 2>/dev/null)

if [ $? -ne 0 ]; then
    echo "Creating Lambda function: verifier-endpoint"
    LAMBDA_ARN=$(aws lambda create-function \
        --function-name verifier-endpoint \
        --runtime nodejs20.x \
    ...
    echo "Updating Lambda function: verifier-endpoint"
    ...
    LAMBDA_ARN=$(aws lambda get-function --function-name verifier-endpoint --query 'Configuration.FunctionArn' --output text --region $AWS_DEFAULT_REGION)
    echo "Lambda function updated successfully"
fi

# API Gatewayの作成または、更新
echo "Checking if API Gateway exists: verifier-endpoint"
API_ID=$(aws apigateway get-rest-apis --query "items[?name=='verifier-endpoint'].id" --output text --region $AWS_DEFAULT_REGION)

if [ -z "$API_ID" ];then
    echo "Creating API Gateway: verifier-endpoint"
